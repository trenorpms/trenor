...
export default function BatchReconcileModal({ isOpen, onClose, onSuccess, invoiceIds, totalDue }: BatchReconcileModalProps) {
  const [description, setDescription] = useState<string>('Batch balance reconciliation payment');
  ...
  useEffect(() => {
    setDescription('Batch balance reconciliation payment');
    setReceiptUrl('');
    setError(null);
  }, [invoiceIds, isOpen]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://localhost:4000/api/properties/invoices/batch-reconcile`, {
        method: 'POST',
        headers: {
      ...
      if (res.ok) {
        ...
        setError('Failed to process batch reconciliation.');
      }
    } catch (err) {
      console.error(err);
    ...
    <div className="fixed inset-0 bg-black/75 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-fadeIn">
      <div className="bg-[#080c10] border border-zinc-800 w-full max-w-md rounded-xl p-6 shadow-2xl relative overflow-hidden font-sans text-xs">
        ...
        <h3 className="font-heading text-sm font-bold text-white uppercase tracking-wider mb-4 border-b border-zinc-850 pb-3">
          Batch Reconcile Selected Invoices
        </h3>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            <label className="text-[9px] text-zinc-400 uppercase font-extrabold tracking-wider">Selected Invoices Count</label>
            <div className="p-3 bg-[#0a0f14] border border-zinc-850 text-white font-extrabold rounded-lg text-xs shadow-inner">
              {invoiceIds.length} statement(s)
            </div>
          </div>

          <div className="flex flex-col gap-1.5">
            <label className="text-[9px] text-zinc-400 uppercase font-extrabold tracking-wider">Total Statement Arrears</label>
            <div className="p-3 bg-[#0a0f14] border border-zinc-850 text-emerald-400 font-extrabold rounded-lg text-sm shadow-inner">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <textarea 
              required 
              rows={2} 
              placeholder="Provide a payment reference or note for these invoices..." 
              value={description} 
          ...
          <div className="flex flex-col gap-2">
            <label className="text-[9px] text-zinc-400 uppercase font-extrabold tracking-wider">Upload Shared Receipt</label>
            <div className="relative">
              <input 
                ...
                className="hidden" 
                id="batch-receipt-file-picker"
              />
              <label 
                htmlFor="batch-receipt-file-picker" 
          ...
          <div className="flex justify-end gap-2.5 mt-2 border-t border-zinc-850 pt-4">
            ...
            <button 
              type="submit" 
              disabled={submitting || uploading || invoiceIds.length === 0} 
            ...
            >
              {submitting ? 'BATCH RECONCILING...' : 'RECONCILE ALL'}
            </button>
          </div>
        </form>
      </div>
